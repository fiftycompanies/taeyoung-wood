...
let dep;
try {
  // DEPLOYMENT_ID 는 시험용 — 지난 배포를 지정해 검출 경로를 실데이터로 확인할 때만 쓴다.
  if (process.env.DEPLOYMENT_ID) {
    dep = await api(`/v13/deployments/${process.env.DEPLOYMENT_ID}`);
    dep.uid = dep.id || dep.uid;
  } else {
    const list = await api(`/v6/deployments?projectId=${PROJECT}&target=production&limit=1`);
...
console.log(`대조 ${compared}개 · 다름 ${diff.length}개 · 배포본에 없는 소스 ${missing.length}개 · 추적 밖 ${untrackedExtra.length}개`);
