...
You are about to reverse-engineer and rebuild **the target URL provided by the user**. **REVRUN 파이프라인 기본 = Phase 1.5 divergence**: 콘텐츠·정보구조는 원본과 1:1 충실하되 시각 표현(히어로·색·폰트)은 REVRUN 고유로 재구성한다(저작권 구조베낌 원천제거). 순수 1:1 픽셀복제가 목적이면 fidelity=pixel-perfect로 지정해 divergence를 끈다.
